...
function insertAudioAndTitle(
  AUDIO_FILEPATH,
  AUDIO_TRACK_NUMBER,
  VIDEO_TRACK_NUMBER,
  SUBTITLE_TEXT,
  SUBTITLE_DURATION_BUFFER,
  MGT_FILE_PATH,
) {
  try {
    // プロジェクトにファイルをインポート
    ...
    var AUDIO_DURATION =
      IMPORTED_ITEM.getOutPoint().seconds -
      IMPORTED_ITEM.getInPoint().seconds +
      SUBTITLE_DURATION_BUFFER;
    ...
    if (IMPORTED_ITEM) {
      ...
      try {
        ...
        activeSequence.audioTracks[AUDIO_TRACK_NUMBER].overwriteClip(
          IMPORTED_ITEM,
          currentCTI.ticks,
        ); // 返り値はboolean
      } catch (e) {
      ...
      try {
        ...
        if (!mogrt) {
          throw new Error(
            "mogrtファイルが見つかりませんでした。NodeIDが存在するか確認してください。",
          );
        }
      } catch (e) {
...
function showMogrtPropList(filePath, videoTrackNumber) {
  // mogrtをシーケンスに配置
  var insertedMogrt = app.project.activeSequence.importMGT(
    filePath,
    getCurrentCTI().ticks,
    videoTrackNumber,
    0,
    ...
    for (var j = 0; j < comp.properties.numItems; j++) {
      alert(
        "Comp No. " +
          ...
          j +
          "プロパティ名: " +
          comp.properties[j].displayName +
          " プロパティの値: " +
          comp.properties[j].getValue(),
      );
    }
  }
}
...
function importMGTtoProj(mogrtFilePath) {
  try {
    insertedMogrt = app.project.activeSequence.importMGT(
      mogrtFilePath,
      getCurrentCTI().ticks,
      1,
      0,
